...
## 论文检索 A/B 指标（OpenAlex 扩源评测）
- 测试脚本：`scripts/ab_paper_search_eval.py`
- 查询集：10 条健康相关 query（HRV、睡眠、压力、焦虑、CBT 等）
- 每源返回：10 条
- 说明：Semantic Scholar 出现 429 限流，现有链路指标偏保守
...
结论：
- OpenAlex 提升相关性均值，但新鲜度偏低（偏经典/高被引论文）。
- 现有链路更“新”，覆盖率中等；加入 OpenAlex 后覆盖率显著提升，但去重率上升。
- 扩源最适合“补全视角”，排序策略保持现有权重不变。
