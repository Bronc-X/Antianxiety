...
### 🛠️ 已采取纠偏措施
- ✅ 将“Build Reliability Protocol”写入 `docs/project/PROJECT_CONSTITUTION.md`。  
...
#### 1. 域名策略统一 (Domain Strategy) ✅
- ✅ **单域名架构**: 采用 `antianxiety.app` 作为主入口，移除基于子域名的强制跳转逻辑。
- ✅ **智能语言检测**: 修改 `lib/subdomain-utils.ts`，优先识别 `zh.`/`en.` 子域名入口，同时支持 cookie (`NEXT_LOCALE`) 和浏览器语言自动检测。
- ✅ **多入口支持**: 在 Vercel 添加 `zh.antianxiety.app` 和 `en.antianxiety.app` 作为独立入口 (No Redirect)，实现多语言版本的直接访问。

#### 2. Vercel 部署修复 ✅
- ✅ **Server/Client 组件分离**: 修复 `refresh-trigger.ts` 在客户端组件中调用服务器端 Supabase 客户端的构建错误。
- ✅ **Cron Job 优化**: 调整 `vercel.json`，将 Cron 任务数量减少至 2 个以符合 Pro 计划限制。
- ✅ **环境变量配置**: 确认 `NEXT_PUBLIC_SITE_URL` 和 Supabase Auth 回调地址配置正确。

#### 3. 视觉资产生成 (Launch Assets) ✅
- ✅ **Silent Luxury 海报**: 设计并生成了 3 张 V5 版本的 HTML 启动海报，采用"Art Gallery"极简美学。
  - *The Sphere* (Global Wisdom)
  - *The Horizon* (Clarity)
  - *The Monolith* (Strategy)
- ✅ **数字孪生形象**: 生成了高质量的 Black Female Digital Twin 图像，采用有机的"Coastline"过渡效果。
- ✅ **社交媒体文案**: 完成了针对朋友圈、小红书 (Build in Public)、X 和 Reddit 的发布文案。

### 📊 代码统计
- **修改文件**: `middleware.ts`, `lib/subdomain-utils.ts`, `lib/auth-utils.ts`, `lib/reddit-auth.ts`, `lib/wechat-auth.ts`, `vercel.json`
- **主要工作**: 基础设施配置与构建修复

### 🚀 下一步
- [x] 正式开启宣传周期 (Marketing Launch)
- [ ] 监控生产环境日志
- [ ] 收集首批用户反馈
...
#### 1. 推荐理由生成逻辑完全重写 ✅
- ✅ **移除所有模板化语句** - 不再出现"先选一个指标调整"等固定文案
- ✅ **移除虚假关注声明** - 不再声称用户"关注"了他们从未选择的内容
- ✅ **数据真实性原则** - 推荐理由只引用用户实际填写的数据：
  - 问卷评估结果 (GAD-7, PHQ-9, ISI 量表分数)
  - 每日校准记录 (睡眠时长、压力评分、能量等级)
  - 主动问询回答 (Max 对话中的用户反馈)
- ✅ **诚实回退机制** - 无数据时显示："这是一篇关于健康科学的内容。完成临床评估和每日记录后，我们会根据你的实际数据推荐更相关的内容。"

#### 2. 推荐系统监控日志 ✅
- ✅ 添加详细的用户数据摘要日志
- ✅ 记录标签生成过程 (userTags 数组)
- ✅ 记录搜索关键词展开过程
- ✅ 记录 PubMed/Semantic Scholar/YouTube 抓取结果

#### 3. News Feed UI 改进 ✅
- ✅ **平台 Logo** - 替换文字标签为官方 SVG 标识
  - PubMed、Semantic Scholar、YouTube、X、Reddit
- ✅ **移除 AI 头像** - Max 推荐理由前不再显示 AI 图标
- ✅ **高度增加** - Landing 页面 News Feed 高度增加 40%
- ✅ **移动端适配** - 信任标识垂直堆叠

#### 4. 宪法更新 ✅
- ✅ 新增 "II-B. Personalized Recommendation Principles" 章节
- ✅ 定义数据真实性原则、诚实回退原则、抓取逻辑原则

### 📊 代码统计
- **修改文件**: 
  - `app/api/curated-feed/route.ts` - 推荐算法重构
  - `components/InfiniteNewsFeed.tsx` - UI 改进
  - `components/DevTools.tsx` - 修复文本选择问题
  - `docs/project/PROJECT_CONSTITUTION.md` - 宪法更新
- **新增代码**: ~200 行
- **删除代码**: ~100 行 (模板化逻辑)
...
## 2025-12-23 - Beta Page 界面重构与核心功能补全 🎨

### 🎯 核心更新

#### 1. 营销页 (Landing Page) 深度重构 ✅
- ✅ **界面重写**: 完全重写 `app/beta/page.tsx`，解决之前的样式和逻辑问题。
- ✅ **视觉升级**:
  - 全局 **Midnight Blue (午夜蓝)** 深色主题，替代压抑的纯黑背景。
  - **极光 (Aurora)** 动态背景与 **Film Noise (胶片噪点)** 纹理叠加，营造电影感。
  - 按钮与卡片增加精细的 **Glassmorphism (玻璃拟态)** 和 **Inner Glow (内发光)** 效果。
- ✅ **交互细节**:
  - 核心功能卡片点击弹窗 (Modal)，展示详细设计理念。
  - 按钮悬停流光效果。

#### 2. 核心功能板块补全 ✅
新增了 3 个关键差异化功能展示：
- ✅ **主动式 AI 诊疗 (Active Inquiry)**: 强调"不只被动记录，更会主动关怀"。
- ✅ **精准科研情报 (Curated Research)**: 强调"基于用户画像的循证医学推荐"。
- ✅ **全生态设备支持 (Device Support)**: 明确"无需购买新硬件"，支持 Apple Watch/华为/小米 等主流设备。

#### 3. 资源修复 ✅
- ✅ 修复 `bg-noise` 引用 404 问题，改用内联 SVG Data URI。

### 📊 代码统计
- **修改文件**: `app/beta/page.tsx` (重构), `components/FeatureCards.tsx`
- **新增代码**: ~300 行 (UI 逻辑与样式)
...
#### 2. 文档维护
- ✅ 更新开发日志
- ✅ 检查 docs/project/TECH_STACK_AND_WORKFLOW.md
...
### 📁 相关文件
- `README.md` - 主要更新文件
- `docs/project/DEVELOPMENT_DIARY.md` - 本日志文件（已更新）
...
## 2026-02-02 - Max 强制远程 AI 接入与错误透出 ✅

### ✅ 修复内容
- Max 对话/起始问题改为**仅允许远程 App API**，本地地址一律报错，避免无 AI 介入的“假成功”。
- 增加 `appApiRequiresRemote` 错误类型，并在 UI 中直接显示具体失败原因，方便定位（网络/配置/远程不可达）。
- Max 请求不再自动切换到本地 API，确保对话必须走远程中转站链路。

### 🧪 运行提示
- Max 必须连接远程 `https://antianxiety.app`；若 APP_API_BASE_URL 仍指向本地，将直接报错提示。
...
## 2026-02-03 - iOS 主动问询同步 & Max 沉浸式布局（未完结）⚠️

### ✅ 已完成
- **主动问询**：移植 Web 逻辑（20 分钟冷却、当天已答缺口过滤、回答写回 `daily_calibrations`）。
- **全局返回键色调**：`ContentView` 增加 `.tint(.liquidGlassAccent)`。
- **校准页布局**：欢迎页按钮缩小/居中、文字与图标对齐调整。
- **Max 沉浸式 UI**：自定义 Header、底部对话框、历史抽屉、Max 页隐藏底部 TabBar。
- **中轴线修正**：科学期刊与数字孪生标题居中（自定义 Header）。
- **构建**：`iostest` 构建通过。

### ⚠️ 未完成（延期到明天）
- **TabBar 漂移/右侧越界**：仍存在“缓慢向右漂移”问题，已尝试等分布局与对称内边距，未彻底修复。
  - 计划：用固定中轴线约束 + 容器宽度锁定方式彻底修正，避免后续再漂移。
