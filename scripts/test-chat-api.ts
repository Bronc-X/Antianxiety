...
async function testChatAPI() {
  ...
  try {
    ...
    // 检查自定义 headers
    const papers = response.headers.get('x-neuromind-papers');
    const consensus = response.headers.get('x-neuromind-consensus');
    const searchStatus = response.headers.get('x-neuromind-search-status');
    
    console.log('\n📋 响应 Headers:');
    console.log(`  x-neuromind-papers: ${papers ? '✅ 有数据' : '❌ 无数据'}`);
    console.log(`  x-neuromind-consensus: ${consensus ? '✅ 有数据' : '❌ 无数据'}`);
