...
describe('Capacitor Configuration Validation', () => {
  ...
  it('should ensure build-capacitor script runs dynamic build and sync', () => {
    const buildScriptPath = path.resolve(__dirname, '../../scripts/build-capacitor.js');
    const scriptContent = fs.readFileSync(buildScriptPath, 'utf-8');

    expect(scriptContent).toContain('动态运行模式');
    expect(scriptContent).toContain('.next');
...
describe('Build Process Integration Tests (Requirements 1.4)', () => {
  ...
  it('should have android assets synced after build', () => {
    const assetsDir = path.join(projectRoot, 'android', 'app', 'src', 'main', 'assets');
    
    // 验证 capacitor 配置文件
    expect(fs.existsSync(path.join(assetsDir, 'capacitor.config.json'))).toBe(true);
    expect(fs.existsSync(path.join(assetsDir, 'capacitor.plugins.json'))).toBe(true);
  ...
  it('should have all required Capacitor plugins configured', () => {
