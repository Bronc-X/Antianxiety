...
async function fetchPubMedContent(
    ...
    try {
        ...
        // TODO: Import and call actual PubMed search
        // For now, return placeholder to avoid circular deps

        // Placeholder implementation - integrate with existing pubmed.ts
        const { searchPubMed } = await import('./services/pubmed');
        const papers = await searchPubMed(query, limit);

        return papers.map(paper => ({
            id: `pubmed_${paper.pmid}`,
            ...
            publishedAt: paper.pubDate || new Date().toISOString(),
            relevanceScore: 0.8, // Papers have high base relevance
...
async function fetchSemanticScholarContent(
    ...
    try {
        ...
        // Use existing Semantic Scholar search
        const { searchSemanticScholar } = await import('./services/semantic-scholar');
