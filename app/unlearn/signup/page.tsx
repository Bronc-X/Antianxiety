...
export default function SignupPage() {
  ...
  const handleSignup = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (success) {
        // useAuth handles redirection to /unlearn/onboarding
  ...
  const handleOAuthSignup = async (provider: 'twitter' | 'github' | 'wechat') => {
    ...
    setMessage(null);
    try {
      const redirectTo = `${window.location.origin}/auth/callback?next=/unlearn/onboarding`;
      ...
      <div className="w-full max-w-md space-y-8">
        ...
        <AnimatedSection variant="fadeUp" className="mt-8">
          ...
          <p className="mt-6 text-center text-sm text-[#0B3D2E]/70 dark:text-neutral-400">
            {t('signup.hasAccount')}{' '}
            <Link href="/unlearn/login" className="font-medium text-[#0B3D2E] dark:text-white hover:text-[#0B3D2E]/80 dark:hover:text-neutral-300 underline">{t('signup.loginNow')}</Link>
          </p>
