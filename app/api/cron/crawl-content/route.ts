  ...
  try {
    ...
    if (isWeekend) {
      ...
      return NextResponse.json({
        ...
        mode: 'full',
        message: `周末完整爬取：PubMed ${result.pubmedCount} 篇，Semantic Scholar ${result.semanticCount} 篇，OpenAlex ${result.openalexCount} 篇，Reddit ${result.redditCount} 条，X ${result.xCount} 条`,
      ...
      return NextResponse.json({
        success: result.success,
        mode: 'quick',
        message: `工作日快速爬取：${result.count} 条内容（OpenAlex ${result.openalexCount}，Reddit ${result.redditCount}，X ${result.xCount}）`,
        count: result.count,
        openalexCount: result.openalexCount,
        redditCount: result.redditCount,
        xCount: result.xCount,
