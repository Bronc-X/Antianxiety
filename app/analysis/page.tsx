...
/**
 * AI 代谢分析报告页面
 * 展示用户的代谢指标雷达图、健康短板分析、AI策略生成和抗衰食材推荐
 * 路径: /analysis
 * 
 * CRITICAL: 严格遵守数据完整性原则
 * - 没有真实日志数据时，显示空状态
 * - 不使用问卷答案伪造睡眠/压力数据
 */
...
import { getRadarChartData, type DailyLog } from '@/lib/data-mapping';
import EmptyRadarChart from '@/components/EmptyRadarChart';

// 临时模拟数据（仅用于未登录或加载失败时）
const mockData = [
  { subject: '睡眠恢复', A: 65, fullMark: 100 },
  { subject: '压力管理', A: 40, fullMark: 100 },
  { subject: '能量水平', A: 55, fullMark: 100 },
  { subject: '运动表现', A: 70, fullMark: 100 },
  { subject: '水分摄入', A: 80, fullMark: 100 },
  { subject: '整体健康', A: 62, fullMark: 100 },
];
    ...
    <div className="min-h-screen bg-[#FAF6EF] p-6 md:p-12 font-sans text-[#0B3D2E]">
      ...
      <div className="max-w-4xl mx-auto mb-6">
        <button
          ...
          className="flex items-center gap-2 text-sm text-[#0B3D2E]/60 hover:text-[#0B3D2E] transition-colors"
        >
