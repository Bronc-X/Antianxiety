...
import { Settings, ChevronRight, Moon, Bell, Shield, Loader2 } from "lucide-react";
...
export default function ProfilePage() {
    ...
    const { weeklyLogs, isLoading: isDashboardLoading } = useDashboard();

    const isLoading = isProfileLoading || isDashboardLoading;

    // Calculate stats from logs
    const streak = weeklyLogs.length; // Simplified streak calculation
    const totalMinutes = weeklyLogs.reduce((acc: number, log) => acc + (log.mindfulness_minutes || 0), 0);
            ...
            <div className="flex-1 overflow-y-auto px-6 pb-32 no-scrollbar">
                ...
                <div className="flex flex-col items-center py-8">
                    <div className="relative w-32 h-32 flex items-center justify-center">
                        ...
                        <div className="absolute bottom-0 bg-slate-900 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-lg">
                            LV. {Math.floor(streak / 7) + 1}
                        </div>
                    </div>
                    <h2 className="mt-4 text-xl font-bold text-slate-900">
                        {profile?.full_name || profile?.first_name || 'Friend'}
                    </h2>
                    <p className="text-slate-400 text-sm">
                        {profile?.primary_goal ? `Goal: ${profile.primary_goal}` : '平静探索者'}
                    </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                    <div className="bg-white p-5 rounded-[2rem] shadow-sm">
                        <div className="text-3xl font-bold text-slate-900 mb-1">{streak}</div>
                        <div className="text-xs text-slate-400">连续天数</div>
                    </div>
                    <div className="bg-white p-5 rounded-[2rem] shadow-sm">
                        <div className="text-3xl font-bold text-slate-900 mb-1">{totalMinutes}</div>
                        <div className="text-xs text-slate-400">正念分钟</div>
                    </div>
                </div>

                {/* Menu List - Simplified Navigation */}
                <div className="bg-white rounded-[2rem] p-2 shadow-sm space-y-1">
                    {[
                        { icon: Moon, label: "深色模式", value: "关", href: "/settings?tab=body" },
                        { icon: Bell, label: "提醒设置", value: "每日", href: "/settings?tab=account" },
                        { icon: Shield, label: "隐私安全", value: "", href: "/settings?tab=account" },
                    ].map((item, i) => (
                        <Link key={i} href={item.href || '#'} className="flex items-center justify-between p-4 hover:bg-slate-50 rounded-2xl transition-colors cursor-pointer group">
