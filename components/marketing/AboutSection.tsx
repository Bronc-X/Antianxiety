...
export default function AboutSection() {
    ...
    return (
        <section className="py-24 px-6 md:px-12 max-w-[1400px] mx-auto bg-white dark:bg-[#1A1A1A] transition-colors overflow-hidden relative">
            {/* Background Gradient */}
            ...
            <div className="grid lg:grid-cols-2 gap-20 items-center">
                ...
                >
                    ...
                    <h2 className="font-heading text-5xl md:text-6xl text-[#1A1A1A] dark:text-white leading-[1.1] mb-8 tracking-tight">
                        {language === 'en' ? (
                            <>A <span className="font-serif italic text-[#D4AF37] relative inline-block">Bio-Rhythm
                                <svg className="absolute -bottom-2 w-full h-3 text-[#D4AF37]/30" viewBox="0 0 100 10" preserveAspectRatio="none">
                                    <path d="M0 5 Q 50 10 100 5" stroke="currentColor" strokeWidth="2" fill="none" />
                                </svg>
                            </span> New Paradigm.</>
                        ) : (
                            <>我们致力于开启一场<span className="font-serif italic text-[#D4AF37] text-4xl md:text-5xl relative inline-block mx-2">生理节律
                                <svg className="absolute -bottom-2 w-full h-3 text-[#D4AF37]/30" viewBox="0 0 100 10" preserveAspectRatio="none">
                                    <path d="M0 5 Q 50 10 100 5" stroke="currentColor" strokeWidth="2" fill="none" />
                                </svg>
                            </span>的新范式。</>
                    ...
                    <motion.p
                        ...
                        className="text-lg md:text-xl text-[#1A1A1A]/70 dark:text-white/70 leading-relaxed mb-12 max-w-xl font-light"
                    >
                        {language === 'en'
                    ...
                    <motion.div
                        ...
                        className="flex items-center gap-5 p-6 bg-[#FAF6EF] dark:bg-white/5 border border-[#D4AF37]/20 rounded-2xl shadow-lg shadow-[#D4AF37]/5 relative overflow-hidden group"
                    >
                        ...
                        <p className="text-sm md:text-base font-medium text-[#1A1A1A] dark:text-white leading-relaxed relative z-10">
                            {language === 'en'
                ...
                <div className="grid gap-6">
                    {values.map((v, i) => (
                        <motion.div
                            ...
                            className="flex gap-6 p-8 rounded-3xl bg-white dark:bg-white/5 border border-transparent hover:border-[#D4AF37]/20 transition-all duration-300 shadow-sm hover:shadow-xl hover:shadow-[#D4AF37]/5 group cursor-default"
                        >
                            <div className="w-14 h-14 rounded-2xl bg-[#1A1A1A] dark:bg-white flex items-center justify-center text-white dark:text-black shrink-0 group-hover:bg-[#D4AF37] group-hover:text-white transition-all duration-300 shadow-lg group-hover:shadow-[#D4AF37]/30 group-hover:rotate-6">
                                <v.icon className="w-7 h-7" />
                            </div>
                            <div>
                                <h4 className="text-xl font-bold text-[#1A1A1A] dark:text-white mb-2 group-hover:text-[#D4AF37] transition-colors">{v.title}</h4>
                                <p className="text-[#1A1A1A]/60 dark:text-white/60 leading-relaxed group-hover:text-[#1A1A1A]/80 dark:group-hover:text-white/80 transition-colors">{v.desc}</p>
                            </div>
                        </motion.div>
                    ))}
                </div>
