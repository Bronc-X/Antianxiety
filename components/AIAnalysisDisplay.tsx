  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex items-start justify-between mb-8">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="px-4 py-3 bg-[#FAF6EF]/80 rounded-lg border border-[#E7E1D6]">
                <div className="text-xs text-[#1a5c4a] font-medium mb-1">{t('Average', '平均值')}</div>
                <div className="text-xl font-bold text-[#0B3D2E]">
              ...
              <div className="px-4 py-3 bg-[#0B3D2E]/5 rounded-lg border border-[#0B3D2E]/20">
                <div className="text-xs text-[#1a5c4a] font-medium mb-1">{t('Percentile', '百分位')}</div>
                <div className="text-xl font-bold text-[#0B3D2E]">
      ...
      {plan.micro_habits && plan.micro_habits.length > 0 && (
        <div className="bg-white border border-[#E7E1D6] rounded-lg p-8 shadow-sm">
          <div className="mb-6">
            <h2 className="text-xl font-semibold text-[#0B3D2E] mb-1">{t('Personalized Action Plan', '个性化行动方案')}</h2>
            <p className="text-[#0B3D2E]/70">{t(`${plan.micro_habits.length} micro-habits tailored for you`, `为您定制的 ${plan.micro_habits.length} 个微习惯`)}</p>
          </div>
          <div className="space-y-4">
            {plan.micro_habits.map((habit, i) => (
              <div key={i} className="border border-[#E7E1D6] rounded-lg p-6 hover:border-[#0B3D2E]/30 transition-colors">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="text-lg font-semibold text-[#0B3D2E] mb-4">{language === 'en' ? (habit.name_en || habit.name) : habit.name}</h3>
                    <div className="space-y-3">
                      <div className="flex items-start gap-3">
                        <span className="text-xs font-medium text-[#0B3D2E]/60 uppercase tracking-wide w-16">{t('TRIGGER', '触发')}</span>
                        <span className="text-[#0B3D2E]/80 flex-1">{language === 'en' ? (habit.cue_en || habit.cue) : habit.cue}</span>
                      </div>
                      <div className="flex items-start gap-3">
                        <span className="text-xs font-medium text-[#0B3D2E]/60 uppercase tracking-wide w-16">{t('ACTION', '行动')}</span>
                        <span className="text-[#0B3D2E] font-medium flex-1">{language === 'en' ? (habit.response_en || habit.response) : habit.response}</span>
                      </div>
                      <div className="flex items-start gap-3">
                        <span className="text-xs font-medium text-[#0B3D2E]/60 uppercase tracking-wide w-16">{t('TIMING', '时机')}</span>
                        <span className="text-[#0B3D2E]/80 flex-1">{language === 'en' ? (habit.timing_en || habit.timing) : habit.timing}</span>
                      </div>
                    </div>
                    <div className="mt-4 pt-4 border-t border-[#E7E1D6]">
                      <div className="text-sm text-[#0B3D2E]/70 leading-relaxed">
                        <span className="font-medium text-[#0B3D2E]">{t('Rationale', '原理')}: </span>{language === 'en' ? (habit.rationale_en || habit.rationale) : habit.rationale}
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <div className="text-center">
        <button
          ...
          className="px-6 py-2.5 bg-[#0B3D2E] text-white rounded-lg font-medium hover:bg-[#0a3629] transition-colors shadow-sm"
        >
