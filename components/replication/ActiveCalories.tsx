...
export const ActiveCalories = () => {
    return (
        <Link href="/replication/mindfulness">
            <div className="p-5 bg-white rounded-[2rem] shadow-sm hover:scale-[1.02] transition-transform cursor-pointer">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 h-12 flex gap-1 items-end">
                        {[20, 45, 30, 60, 40, 50, 20, 30, 45, 60].map((h, i) => (
                            <motion.div
                                key={i}
                                initial={{ height: 0 }}
                                animate={{ height: `${h}%` }}
                                transition={{ duration: 0.5, delay: i * 0.05 }}
                                className={`flex-1 rounded-sm ${i > 7 ? "bg-slate-100" : "bg-teal-400"}`} // Highlight recent
                            />
                        ))}
                    </div>
